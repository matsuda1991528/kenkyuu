...
my $biasMutateParameter;
my $specificMutateParameter;
...
for($biasMutateParameter=0;$biasMutateParameter<=100;$biasMutateParameter+=10){
	$specificMutateParameter = 100 - $biasMutateParameter;
	open(OUT,">mixMutateData/normalMutate_$biasMutateParameter.csv");
    #print OUT "\"estimateUncomParameter\", $estimateUncomParameter\n";                   #mutation_rateをファイル出力	
    close(OUT);
	
	#for($seed=0;$seed<$ind;$seed++){
	$result=system("./go -seed 0 -cross 0.4 -mutate 0.2 -estimate 1 -bias $biasMutateParameter  >> mixMutateData/normalMutate_$biasMutateParameter.csv");  	
	#}
}
...
#for($estimateUncomParameter=1;$estimateUncomParameter<=100;$estimateUncomParameter++){
