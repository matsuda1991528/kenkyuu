...
struct sun_angle_t getSunAngle(double time){
	...
	sun_angle.elevation = 0.0f;
	sun_angle.azimuth = 0.0f;
	...
	solarAngle(y_day, time, &sun_angle.elevation, &sun_angle.azimuth);
	sun_angle.elevation = sun_angle.elevation / fact;
	sun_angle.azimuth = (sun_angle.azimuth / fact) - 270;  //西を0度にする処理
	return sun_angle;	
}
#endif
